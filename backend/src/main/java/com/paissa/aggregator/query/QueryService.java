...
public class QueryService {
     ...
     * {@code ownership} is null), optionally scoped to one datacenter. Every row carries the full
     * small/medium/large breakdown; {@code rankBy} only selects which column worlds are sorted by.
     */
    public List<WorldStats> worldLeaderboard(PlotSize rankBy, PurchaseSystem ownership, Integer datacenterId) {
        ...
        for (WorldSizeCountRow row : plotQueryRepository.countByWorldAndSize(purchaseSystemCodes, datacenterId)) {
    ...
    public Page<Plot> worldPlots(Integer worldId, PlotSize size, PurchaseSystem ownership, Pageable pageable) {
        List<Integer> purchaseSystemCodes = ownership != null ? ownership.rawCodesForTab() : allRawCodes();
        return size != null
                ? plotQueryRepository.findWorldPlotsBySize(worldId, size, purchaseSystemCodes, pageable)
                : plotQueryRepository.findWorldPlots(worldId, purchaseSystemCodes, pageable);
    }
